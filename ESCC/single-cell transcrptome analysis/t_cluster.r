...
unique_periods <- sort(unique(t_cells@meta.data$period))  # 按字母顺序排序
# 如果需要手动指定顺序，可以这样做：
...
for (period in unique_periods) {
    ...
    # 绘制 DimPlot
    p <- DimPlot(t_cells, reduction = "umap", label = FALSE, pt.size = 1, group.by = "period") +
...
# 将 cell_type 列转换为因子，并指定顺序
t_tumor@meta.data$cell_type <- factor(t_tumor@meta.data$cell_type, levels = identity_mapping)

# 获取唯一的细胞类型并转换为字符向量
cell_types <- as.character(unique(t_tumor@meta.data$cell_type))
num_legend_items <- length(cell_types)  # 图例的个数
max_label_length <- max(nchar(cell_types))  # 图例名称的最大长度
...
# 保存图片
png("t_tumor.png", width = dynamic_width, height = base_height, res = 300)
DimPlot(t_tumor, reduction = "umap", label = TRUE, pt.size = 2, label.size = 8, group.by = "cell_type") +
    xlab("UMAP_1") +
    ylab("UMAP_2") +
    ggtitle(NULL) +
    scale_color_npg() +
