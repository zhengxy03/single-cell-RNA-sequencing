...
# 获取epi对象中的基因名
epi_genes <- rownames(epi)

# 匹配去分化基因与epi中的基因
matched_genes <- Dedifferentiation_genes[Dedifferentiation_genes %in% epi_genes]
length(matched_genes)
Dedifferentiation_genes <- matched_genes

# 计算去分化评分
epi <- AddModuleScore(epi, features = list(Dedifferentiation_genes), name = "Dedifferentiation_Score")

# 查看元数据
head(epi@meta.data)

# 标准化评分
epi@meta.data$Dedifferentiation_Score_Z <- scale(epi@meta.data$Dedifferentiation_Score1)
summary(epi@meta.data$Dedifferentiation_Score_Z)
...
p <- FeaturePlot(
  ...
  scale_color_gradient2(
    ...
    mid = "white",      # 中值颜色
    high = "#E64B35FF", # 高值颜色
    midpoint = 0,       # 中值点（对应Dedifferentiation_Score1的中值）
    limits = c(-0.6, 0.8)  # 固定图例范围
  )
ggsave("epi_stem_umap.png", plot = p)
...
p <- VlnPlot(
  object = epi,
  features = "Dedifferentiation_Score_Z",
  group.by = "cell_type2",  # 替换为 cell_type2
  pt.size = 0,
  cols = npg_extended
) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1))
ggsave("epi_stem_vln.png", plot = p)