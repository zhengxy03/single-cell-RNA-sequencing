...
#metLN <- subset(obj, subset = tissue == "metLN")
...
for (sample_id in samples) {
  ...
  # 提取该样本的细胞
  sample_cells <- subset(mPT, subset = sample == sample_id)
...
saveRDS(all_results, file = "mPT_all_results_subtype_interaction.rds")
# ===================== 整合多样本结果 =====================
...
if (length(all_cell_types) >= 2) {
  ...
  p_avg <- pheatmap(avg_log2fc,
                    ...
                    number_color = "black",
                    fontsize_number = 8,
                    filename = "mPT_all_samples_average_contact_log2fc_detailed.pdf",
                    width = 14, height = 12)
  
  cat("保存平均热图：mPT_all_samples_average_contact_log2fc.pdf\n")
  ...
  saveRDS(integrated_results, file = "mPT_integrated_results_detailed.rds")
  cat("保存整合结果：mPT_integrated_results.rds\n")
}
