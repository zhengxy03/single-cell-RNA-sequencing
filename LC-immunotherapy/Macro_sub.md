...
# 先提取Macrophage相关的数据
macro_data <- myeloid@meta.data %>%
  filter(major_cell == "Macro") %>%  # 只保留Macrophage
  group_by(Group, major_cell, cell_type) %>%
  summarise(count = n()) %>%
  group_by(Group, major_cell) %>%
  mutate(proportion = count / sum(count)) %>%
  ungroup() %>%
  mutate(group_major = paste0(Group, "_", major_cell)) %>%
  mutate(group_major = factor(group_major,
                             levels = c("Group1_Macro", "Group2_Macro")))

# 画Macrophage的饼图
pdf("myeloid_Group_Macro_cell_type_pie.pdf", width = 4000/300, height = 3000/300) 
...
npg_extended <- colorRampPalette(npg_pal)(14)
# 再提取DC相关的数据
dc_data <- myeloid@meta.data %>%
  filter(major_cell == "DC") %>%  # 只保留DC
  group_by(Group, major_cell, cell_type) %>%
  summarise(count = n()) %>%
  group_by(Group, major_cell) %>%
  mutate(proportion = count / sum(count)) %>%
  ...
  mutate(group_major = paste0(Group, "_", major_cell)) %>%
  mutate(group_major = factor(group_major,
                             levels = c("Group1_DC", "Group2_DC")))

# 画DC的饼图
pdf("myeloid_Group_DC_cell_type_pie.pdf", width = 4000/300, height = 3000/300) 
...
# 绘制分面柱状图
pdf("macro_group_response_proportion.pdf", width = 6000/300, height = 2000/300)
...
p <- ggplot(cell_proportions_complete, aes(x = cell_type, y = proportion, fill = cell_type)) +
  ...
  facet_wrap(~ Group_Response, ncol = 4, scales = "free_x")  # 改为2列布局
...
# 绘制分面柱状图
pdf("DC_group_response_proportion.pdf", width = 6000/300, height = 2000/300)
...
# ===== 第一部分：准备柱状图数据 =====
cell_proportions_bar <- macro@meta.data %>%
  group_by(Group, Response, cell_type) %>%
  summarise(count = n(), .groups = 'drop') %>%
  group_by(Group, Response) %>%
  mutate(proportion = count / sum(count)) %>%
...
plot_data_box_combined <- plot_data_box_combined %>%
  arrange(Group, cell_type, Response) %>%
  mutate(
    Response = factor(Response, levels = c("YES", "NO")),
    # 创建排序因子，确保每个细胞类型内YES在前
    order_factor = interaction(cell_type, Response, sep = "_")
  )
...
# 【关键修改2】创建箱图（右侧）- 明确指定分组顺序
box_plot <- ggplot(plot_data_box_combined, aes(x = cell_type, y = proportion, fill = Response)) +
  # 明确指定分组，确保YES在左，NO在右
  geom_boxplot(width = 0.7, aes(group = interaction(cell_type, Response))) +
  # 为0值数据添加横线
  geom_segment(
...
# 保存图形
pdf("macro_combined_bar_box_plots.pdf", width = 6000/300, height = 3000/300)
print(final_plot)
dev.off()
...
npg_extended <- colorRampPalette(npg_pal)(14)
selected_colors <- npg_extended[6:13]
pdf("macro_combined_annotation_by_group.pdf", width = 4000/300, height = 2000/300)
...
pdf("macro_box_proportion_by_group.pdf", width = 6000/300, height = 3000/300)
ggplot(plot_data, aes(x = cell_type, y = proportion, fill = Group)) +
  geom_boxplot(width = 0.7) +
  geom_text(
    data = chisq_results, 
