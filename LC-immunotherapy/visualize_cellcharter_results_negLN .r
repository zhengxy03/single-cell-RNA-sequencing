...
# 确保邻域标签是因子
negLN$cellcharter_cluster <- as.factor(negLN$cellcharter_cluster)

# 获取邻域数量
n_clusters <- length(unique(negLN$cellcharter_cluster))
#n_clusters <- length(unique(nmPT$cellcharter_cluster))
message(paste("邻域数量:", n_clusters))

# 检查是否有sample列
has_sample <- "sample" %in% colnames(negLN@meta.data)
if (!has_sample) {
  message("未找到sample列，将创建默认sample")
  negLN@meta.data$sample <- "all_samples"
}
...
has_spatial <- x_col %in% colnames(negLN@meta.data) && y_col %in% colnames(negLN@meta.data)

if (has_spatial) {
  message("使用空间坐标进行可视化")
  coords <- negLN@meta.data[, c(x_col, y_col)]
  colnames(coords) <- c("COORD_1", "COORD_2")
  negLN[['spatial_coords']] <- CreateDimReducObject(
  ...
  if (!"umap" %in% names(negLN@reductions)) {
    negLN <- RunUMAP(negLN, dims = 1:50)
  }
  reduction_name <- "umap"
}

samples <- unique(negLN@meta.data$sample)
...
for (sample in samples) {
  ...
  sample_data <- negLN@meta.data[negLN@meta.data$sample == sample, ]
  sample_cells <- rownames(sample_data)
  temp_seurat <- subset(negLN, cells = sample_cells)
  ...
  sample_filename <- paste0("negLN_spatial_domain_sample_", gsub(" ", "_", sample), ".pdf")
...
if (has_spatial) {
  ...
  ggsave("negLN_spatial_domain_all_samples.pdf", p_combined, width = 14, height = 10, dpi = 300)
  message("保存合并空间图: negLN_spatial_domain_all_samples.pdf")
}
# =====================
# 3. 每个邻域细胞组成热图
# =====================
