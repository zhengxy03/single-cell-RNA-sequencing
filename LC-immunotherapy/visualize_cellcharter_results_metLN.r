...
# 确保邻域标签是因子
metLN$cellcharter_cluster <- as.factor(metLN$cellcharter_cluster)

# 获取邻域数量
n_clusters <- length(unique(metLN$cellcharter_cluster))
#n_clusters <- length(unique(nmPT$cellcharter_cluster))
message(paste("邻域数量:", n_clusters))

# 检查是否有sample列
has_sample <- "sample" %in% colnames(metLN@meta.data)
if (!has_sample) {
  message("未找到sample列，将创建默认sample")
  metLN@meta.data$sample <- "all_samples"
}
...
has_spatial <- x_col %in% colnames(metLN@meta.data) && y_col %in% colnames(metLN@meta.data)

if (has_spatial) {
  message("使用空间坐标进行可视化")
  coords <- metLN@meta.data[, c(x_col, y_col)]
  colnames(coords) <- c("COORD_1", "COORD_2")
  metLN[['spatial_coords']] <- CreateDimReducObject(
  ...
  if (!"umap" %in% names(metLN@reductions)) {
    metLN <- RunUMAP(metLN, dims = 1:50)
  }
  reduction_name <- "umap"
}

samples <- unique(metLN@meta.data$sample)
...
for (sample in samples) {
  ...
  sample_data <- metLN@meta.data[metLN@meta.data$sample == sample, ]
  sample_cells <- rownames(sample_data)
  temp_seurat <- subset(metLN, cells = sample_cells)
  ...
  sample_filename <- paste0("metLN_spatial_domain_sample_", gsub(" ", "_", sample), ".pdf")
...
if (has_spatial) {
  ...
  ggsave("metLN_spatial_domain_all_samples.pdf", p_combined, width = 14, height = 10, dpi = 300)
  message("保存合并空间图: metLN_spatial_domain_all_samples.pdf")
}
