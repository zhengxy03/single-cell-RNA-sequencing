...
# 检查是否有sample列
has_sample <- "sample" %in% colnames(nmPT@meta.data)
if (!has_sample) {
  message("未找到sample列，将创建默认sample")
  nmPT@meta.data$sample <- "all_samples"
}
...
has_spatial <- x_col %in% colnames(nmPT@meta.data) && y_col %in% colnames(nmPT@meta.data)

if (has_spatial) {
  message("使用空间坐标进行可视化")
  coords <- nmPT@meta.data[, c(x_col, y_col)]
  colnames(coords) <- c("COORD_1", "COORD_2")
  nmPT[['spatial_coords']] <- CreateDimReducObject(
  ...
  if (!"umap" %in% names(nmPT@reductions)) {
    nmPT <- RunUMAP(nmPT, dims = 1:50)
  }
  reduction_name <- "umap"
}

samples <- unique(nmPT@meta.data$sample)
...
for (sample in samples) {
  ...
  sample_data <- nmPT@meta.data[nmPT@meta.data$sample == sample, ]
  sample_cells <- rownames(sample_data)
  temp_seurat <- subset(nmPT, cells = sample_cells)
  ...
  sample_filename <- paste0("nmPT_spatial_domain_sample_", gsub(" ", "_", sample), ".pdf")
